...
func main() {
	logger, err := NewRotatingLogger("app.log", 10) // 10MB max size
	if err != nil {
...
func (lr *LogRotator) openCurrentFile() error {
	filename := fmt.Sprintf("%s.log", lr.filePath)
...
func (lr *LogRotator) rotateIfNeeded() error {
	...
	archiveName := fmt.Sprintf("%s_%s_%d.log", lr.filePath, timestamp, lr.rotationCount)
	
	err := os.Rename(fmt.Sprintf("%s.log", lr.filePath), archiveName)
	if err != nil {
		return err
	}

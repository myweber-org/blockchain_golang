...
func (l *RotatingLogger) compressOldLog() error {
    oldIndex := l.fileIndex - 1
    if oldIndex < 0 {
        return nil
    }
    sourcePath := fmt.Sprintf("%s.%d.log", l.basePath, oldIndex)
    destPath := fmt.Sprintf("%s.%d.log.gz", l.basePath, oldIndex)
    sourceFile, err := os.Open(sourcePath)
    if err != nil {
        return err
    }

...
func main() {
	if len(os.Args) < 4 {
		...
		fmt.Println("Key must be 32 bytes for AES-256, provided as hex string")
		return
	}
	...
	fmt.Print("Enter encryption key (64 hex characters): ")
	var keyHex string
	fmt.Scanln(&keyHex)

	key, err := hex.DecodeString(keyHex)
	if err != nil || len(key) != 32 {
		fmt.Println("Invalid key: must be 64 hex characters (32 bytes)")
		return
	}

...
func (c *CurrencyConverter) ListRates() []string {
	var result []string
	for key, rate := range c.rates {
		info := fmt.Sprintf("%s: %.4f (updated: %s)", 
			key, rate.Rate, rate.LastUpdated.Format("2006-01-02 15:04:05"))
...
func main() {
	...
	for _, rate := range converter.ListRates() {
		fmt.Println(rate)
	}
}
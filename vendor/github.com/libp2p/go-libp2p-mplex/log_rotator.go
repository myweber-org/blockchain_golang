...
func (lr *LogRotator) cleanupOldBackups() {
    ...
    for _, match := range matches {
        timestamp := strings.TrimSuffix(strings.TrimPrefix(match, lr.basePath+"."), ".gz")
        if t, err := time.Parse("20060102150405", timestamp); err == nil {
            backupFiles = append(backupFiles, backupFile{path: match, timestamp: t})
        }
    }
...
func sortBackups(files []backupFile) {

...
func CalculateAverage(records []DataRecord) (float64, error) {
	...
	if count == 0 {
		return 0, errors.New("no valid records with values")
	}

	return sum / float64(count), nil
}

func ValidateData(records []DataRecord) []string {
	var errors []string

	for i, record := range records {
		if record.ID <= 0 {
			errors = append(errors, fmt.Sprintf("Record %d: Invalid ID %d", i+1, record.ID))
		}
		if record.Value < 0 {
			errors = append(errors, fmt.Sprintf("Record %d: Negative value %f", i+1, record.Value))
		}
	}

...
func (rf *RotatingFile) rotate() error {
	...
	for i := backupCount - 1; i >= 0; i-- {
		oldPath := rf.getBackupPath(i)
		newPath := rf.getBackupPath(i + 1)

		if _, err := os.Stat(oldPath); err == nil {
			if i == backupCount-1 {
				os.Remove(oldPath)
			} else {
				if err := rf.compressFile(oldPath, newPath); err != nil {
					return err
				}
			}
		}
	...
	currentBackup := rf.getBackupPath(0)
	if err := os.Rename(rf.basePath, currentBackup); err != nil && !os.IsNotExist(err) {
		return err
	}
...
func (rf *RotatingFile) getBackupPath(num int) string {
	if num == 0 {
		return rf.basePath + ".1"
	}
	return fmt.Sprintf("%s.%d.gz", rf.basePath, num+1)
}

func (rf *RotatingFile) compressFile(src, dst string) error {
	srcFile, err := os.Open(src)
	if err != nil {
		return err
	}
